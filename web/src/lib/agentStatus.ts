export type TerminalStatus = 'idle' | 'streaming' | 'done' | 'done_with_warnings' | 'done_with_errors' | 'error' | 'awaiting_approval'
...
export function columnForStatus(status: TerminalStatus): string | null {
  ...
  if (status === 'done_with_warnings') return 'done'
  if (status === 'done_with_errors') return 'done'
