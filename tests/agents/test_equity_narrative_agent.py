"""Unit tests for agents/equity_narrative_agent.py."""
...
class TestCreateEquityNarrative:
    ...
    def test_gathers_context_from_tools_directly_then_forces_schema(
        self,
        mock_search,
        mock_news,
    ):
        ...
        mock_model.bind_tools.return_value = mock_structured
    ...
    @patch("tools.market.news.ingest_stock_news")
    @patch("tools.archivist.search.search_all_memories")
    def test_uses_company_name_in_vault_search_when_available(
        self,
        mock_search,
        mock_news,
    ):
    ...
    @patch("tools.market.news.ingest_stock_news")
    @patch("tools.archivist.search.search_all_memories")
    def test_tool_failure_does_not_crash_still_calls_llm(
        self,
        mock_search,
        mock_news,
    ):
        mock_search.invoke.side_effect = Exception(
            "vault unavailable"
        )
        mock_news.invoke.side_effect = Exception(
            "network error"
        )
